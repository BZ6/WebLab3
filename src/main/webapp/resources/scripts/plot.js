...
function rChanged() {
    document.querySelectorAll('input[name="shot_form:r"]').forEach((rSlider) => {
        sessionStorage.r = rSlider.value
    })
    drawPlot()
}

function getShotR() {
    return sessionStorage.r?.length ? parseFloat(JSON.parse(sessionStorage.r)) : null
}
...
function drawPlot() {
    ...
    // rectangle
    ctx.fillRect(transformX(0), transformY(R), R * scale, R * scale)
    ...
    shotRows.forEach(row => {
        ...
        if (r === shotR && count < 10) {
            drawPoint(x, y, r, cells[3].innerText === "In");
            count++;
        }
    })
